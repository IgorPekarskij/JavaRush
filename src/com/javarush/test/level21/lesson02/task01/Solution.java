...
public class Solution {
    ...
    public static void print(byte[] bytes) {
        for(Byte item: bytes){
            String bynery = Integer.toBinaryString((int)item);
            if(bynery.length() < 8) {
                String result = "";
                for(int i = bynery.length(); i < 8; i++)
                    result += 0;
                System.out.printf("%S ", result + bynery);
            }
            else
                System.out.printf("%S ",bynery.substring(bynery.length()-8));
        }
        System.out.println();
    }
}
